...
# def knn_pytorch(query_pts, ref_pts, k=1, search_radius=None, batch_size=5):
#     """
#     Optimized KNN search using PyTorch with reduced GPU memory usage.
    
#     Args:
#         query_pts (Tensor): (M, 3) tensor (points to find neighbors for)
#         ref_pts (Tensor): (N, 3) tensor (reference points)
#         k (int): Number of nearest neighbors to retrieve
#         search_radius (float, optional): Maximum valid distance threshold
#         batch_size (int): Number of query points processed per batch (reduces memory load)
    
#     Returns:
#         indices (Tensor): (M, k) tensor of nearest neighbor indices
#         distances (Tensor): (M, k) tensor of distances
#     """
...
#         batch_query = query_pts[i:i+batch_size].contiguous()  # (B, 3)
        
#         # Compute pairwise distances for batch
#         dists = torch.cdist(batch_query, ref_pts, compute_mode="donot_use_mm_for_euclid_dist")  # (B, N)
        
#         # Get K nearest neighbors
#         batch_distances, batch_indices = torch.topk(dists, k=k, dim=1, largest=False)
        
#         # Apply search radius mask if needed
#         if search_radius is not None:
#             mask = batch_distances < search_radius
#             batch_indices = torch.where(mask, batch_indices, torch.tensor(-1, dtype=torch.long, device=device))
#             batch_distances = torch.where(mask, batch_distances, torch.tensor(float('inf'), dtype=torch.float32, device=device))
...
def knn_pytorch(query_pts, ref_pts, k=1, search_radius=None):
    """
    query_pts: (M, 3) tensor (points to find neighbors for)
    ref_pts: (N, 3) tensor (reference points)
    Returns: (indices, distances)
    """
    # Compute pairwise distances (ensure float32)
    dists = torch.cdist(query_pts.float(), ref_pts.float())  # <-- Force float32

    # Find K nearest neighbors
    distances, indices = torch.topk(dists, k=k, dim=1, largest=False)
    
    # Apply search radius mask (optional)
    if search_radius is not None:
        mask = distances < search_radius
        # Replace invalid indices with -1 and distances with float32 infinity
        indices = torch.where(mask, indices, torch.tensor(-1, dtype=torch.long, device=indices.device))
        distances = torch.where(mask, distances, torch.tensor(float('inf'), dtype=torch.float32, device=distances.device))  # <-- Use float32
...
def remove_small_group(group_ids, th):
    """
    Remove the group with less than th points

    Args:
        group_ids: np.ndarray, (N,)
        th: int, threshold, when the number of points in a group is less than th, remove the group
    """
...
#     indices, dis = knn_pytorch(gen_coord, scene_coord, k=1, search_radius=search_voxel_size*1.5)
...
def get_matching_indices(pcd0, pcd1, search_voxel_size, K=None, gpu_id=1):
    """
    Finds corresponding points between two Open3D point clouds using nearest neighbor search.

    Args:
        pcd0: Open3D point cloud (reference).
        pcd1: Open3D point cloud (query).
        search_voxel_size: Max distance threshold for valid matches.
        K: Unused parameter (can be removed if not needed).
        gpu_id: GPU device ID to use for computation.

    Returns:
        List of matched point indices [[idx_pcd0, idx_pcd1], ...].
    """
    device = torch.device(f"cuda:{gpu_id}" if torch.cuda.is_available() else "cpu")

    match_inds = []
    
    # Convert Open3D point clouds to PyTorch tensors and move to specified GPU
    scene_coord = torch.tensor(np.asarray(pcd0.points), dtype=torch.float32, device=device).contiguous()
    gen_coord = torch.tensor(np.asarray(pcd1.points), dtype=torch.float32, device=device).contiguous()

    # Find nearest neighbors in pcd0 for each point in pcd1
    indices, dis = knn_pytorch(
        query_pts=gen_coord, 
        ref_pts=scene_coord, 
        k=1, 
        search_radius=1.5 * search_voxel_size
    )

    # Convert to numpy arrays and flatten
    indices = indices.cpu().numpy().flatten()  # Shape (N,)
    dis = dis.cpu().numpy().flatten()          # Shape (N,)

    # Collect valid matches
    for j in range(len(gen_coord)):
        if dis[j] < search_voxel_size and indices[j] != -1:
            i = int(indices[j])  # Index in pcd0
            match_inds.append([i, j])  # [pcd0_index, pcd1_index]
